/* File:     ex6.12b_mpi_nbody_basic.c
 * Purpose:  Implement a 2-dimensional n-body solver that uses the 
 *           basic algorithm.  This version uses an in-place Allgather
 *           It also distributes the masses among the processes.
 *
 * Compile:  mpicc -g -Wall -o ex6.12b_mpi_nbody_basic 
 *                 ex6.12b_mpi_nbody_basic.c -lm
 *           To turn off output (e.g., when timing), define NO_OUTPUT
 *           To get verbose output, define DEBUG
 *
 * Run:      mpiexec -n <number of processes> ./ex6.12b_mpi_nbody_basic
 ...
 *
 * Notes:
 * 1.  The masses are distributed among the processes.
 *
 * IPP:  Exercise 6.12b
 */
#include <stdio.h>
...
void Get_init_cond(double masses[], double loc_masses[], vect_t pos[], 
      vect_t loc_vel[], int n, int loc_n);
void Gen_init_cond(double masses[], double loc_masses[], vect_t pos[], 
      vect_t loc_vel[], int n, int loc_n);
...
void Print_loc_array(double x[], int sz, char title[]);
...
int main(int argc, char* argv[]) {
   ...
   if (g_i == 'i')
      Get_init_cond(masses, loc_masses, pos, loc_vel, n, loc_n);
   else
      Gen_init_cond(masses, loc_masses, pos, loc_vel, n, loc_n);
...
#  endif
   for (step = 1; step <= n_steps; step++) {
      t = step*delta_t;
      MPI_Allgather(loc_masses, loc_n, MPI_DOUBLE, 
            masses, loc_n, MPI_DOUBLE, comm);
...
void Print_loc_array(double x[], int sz, char title[]) {
   ...
   for (i = 0; i < sz; i++)
      sprintf(string + strlen(string), "%.3e ", x[i]);
   printf("%s\n", string);
   fflush(stdout);
}
