/* File:     ex6.12a_mpi_nbody_basic.c
 * Purpose:  Implement a 2-dimensional n-body solver that uses the 
 *           basic algorithm.  This version uses separate arrays
 *           for the positions of the local particles and the
 *           positions of the global particles.
 *
 * Compile:  mpicc -g -Wall -o ex6.12a_mpi_nbody_basic 
 *                 ex6.12a_mpi_nbody_basic.c -lm
 *           To turn off output (e.g., when timing), define NO_OUTPUT
 *           To get verbose output, define DEBUG
 *
 * Run:      mpiexec -n <number of processes> ./ex6.12a_mpi_nbody_basic
...
void Print_vect_t(vect_t x[], int sz, char title[]);
...
int main(int argc, char* argv[]) {
   ...
   memcpy(loc_pos, pos + my_rank*loc_n, 2*loc_n*sizeof(double));
...
/*---------------------------------------------------------------------
 * Function:  Print_vect_t
 * Purpose:   Print an array of vect_t for debugging
 * In args:   all
 */
void Print_vect_t(vect_t x[], int sz, char title[]) {
   char string[1000];
   ...
   sprintf(string, "Proc %d > %s:\n", my_rank, title);
   for (i = 0; i < sz; i++)
      sprintf(string + strlen(string), "   %10.3e %10.3e\n", x[i][X], x[i][Y]);
   printf("%s", string);
}  /* Print_vect_t */
