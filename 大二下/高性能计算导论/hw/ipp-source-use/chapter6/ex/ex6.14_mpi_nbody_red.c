 ...
 *           in Compute_proc_forces.  This version uses two calls
 *           to MPI_Sendrecv_replace in the for loop in Compute_forces.
 *
...
void Compute_forces(double masses[], vect_t tmp_data[], 
   ...
   for (i = 1; i < comm_sz; i++) {
      ...
      MPI_Sendrecv_replace(tmp_data, loc_n, vect_mpi_t, dest, 0, src, 0,
            comm, &status);
      MPI_Sendrecv_replace(tmp_data + loc_n, loc_n, vect_mpi_t, dest, 0, 
            src, 0, comm, &status);
      Compute_proc_forces(masses, tmp_data, loc_forces, loc_pos, loc_n, 
