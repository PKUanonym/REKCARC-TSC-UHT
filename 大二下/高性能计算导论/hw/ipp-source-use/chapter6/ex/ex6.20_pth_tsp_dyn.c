 ...
 *           gives it additional work.  This version implements
 *           four different stack splitting strategies:
 *           (0)  The original splitting strategy that assigns
 *                alternate tours on the donor's stack to the
 *                thread that's run out of work.
 *           (1)  The top k/2 tours on the donating thread's stack
 *                are given to the thread that's run out of work.
 *           (2)  The tours on the donating thread's are split on 
 *                the basis of cost:  least expensive to donor,
 *                next least expensive to thread that's run out
 *                of work, etc.
 *           (3)  The tours on the donating thread's are split on 
 *                the basis of average cost per edge:  least expensive 
 *                to donor, next least expensive to thread that's run out
 *                of work, etc.
 *
 * Compile:  gcc -g -Wall -o ex6.20_pth_tsp_dyn ex6.20_pth_tsp_dyn.c -lpthread
 *           Needs timer.h
 * Usage:    ex6.20_pth_tsp_dyn <thread count> <matrix_file> 
 *              <min split size> <0|1|2|3>
 *
 ...
 *     digraph[i*n + j]
 * 6.  Define SPLIT_DEBUG to test Split_stack: this should call 
 *     Split_stack after the fourth pass through the main while 
 *     loop in Par_tree_search, print the stack, split the stack, 
 *     print the new stacks and quit
 *
 * IPP:  Exercise 6.20
 */
#include <stdio.h>
...
my_stack_t Alternating_split(my_stack_t stack);
my_stack_t Take_top_split(my_stack_t stack);
my_stack_t Cost_split(my_stack_t stack);
my_stack_t Avg_cost_split(my_stack_t stack);
int Compare_tour_cost(const void* t1_p, const void* t2_p);
int Compare_avg_cost(const void* t1_p, const void* t2_p);
int Compare_tour_len(const void* t1_p, const void* t2_p);
...
int main(int argc, char* argv[]) {
   ...
   split_strategy = strtol(argv[4], NULL, 10);
   if (split_strategy < 0 || split_strategy >= 4) {
      fprintf(stderr, "Split strategy should 0, 1, 2, or 3\n");
      Usage(argv[0]);
   }
...
#     ifdef SPLIT_DEBUG
      if (pass == 3) {
         new_stack = Split_stack(stack, my_rank);
         free(new_stack);
         break;
      } else {
         pass++;
...
#     ifdef SPLIT_DEBUG
      sprintf(string+strlen(string), ", cost = %d, count = %d, avg = %f",
            stack->list[i]->cost, stack->list[i]->count,
            ((float)(stack->list[i]->cost))/(stack->list[i]->count-1));
#     endif
      printf("%s\n", string);
   }
...
#  endif
   ...
   switch (split_strategy) {
      case 0:
         new_stack = Alternating_split(stack);
         break;
      case 1:
         new_stack = Take_top_split(stack);
         break;
      case 2:
         new_stack = Cost_split(stack);
         break;
      case 3:
         new_stack = Avg_cost_split(stack);
         break;
   }
 ...
 * Purpose:    Split the stack by assigning alternate records
 *             to the new stack
 * In/out arg: stack
 * Ret val:    new_stack
 */
...
/*------------------------------------------------------------------
 * Function:   Take_top_split
 * Purpose:    Split the stack by assigning the top records in the
 *             current stack to the new stack
 * In/out arg: stack
 * Ret val:    new_stack
 */
my_stack_t Take_top_split(my_stack_t stack) {
   my_stack_t new_stack = Init_stack();
   int new_stack_sz = stack->list_sz/2;
   int i, j;

   for (i = stack->list_sz - new_stack_sz, j = 0; j < new_stack_sz; j++, i++) {
      new_stack->list[j] = stack->list[i];
      stack->list[i] = NULL;
   }
   stack->list_sz -= new_stack_sz;
   new_stack->list_sz = new_stack_sz;
...
/*------------------------------------------------------------------
 * Function:   Compare_tour_cost
 * Purpose:    Compare the cost of two tours and determine which
 *             is smaller
 * In args:    t1_p, t2_p:  pointers to pointers to two tours
 * Ret val:    -1 if cost of *t1_p has lower cost than *t2_p
 *             0  if they have equal cost
 *             1  if *t1_p has greater cost
 * Note:       For use by qsort library function
 */
int Compare_tour_cost(const void* t1_p, const void* t2_p) {
   tour_t t1 = *((tour_t*) t1_p);
   tour_t t2 = *((tour_t*) t2_p);

   if (t1->cost < t2->cost)
      return -1;
   else if (t1->cost == t2->cost)
      return 0;
   else /* t1->cost > t2->cost */
      return 1;
}  /* Compare_tour_cost */


/*------------------------------------------------------------------
 * Function:   Compare_avg_cost
 * Purpose:    Compare the average cost per edge of two tours and 
 *             determine which is smaller
 * In args:    t1_p, t2_p:  pointers to pointers to two tours
 * Ret val:    -1 if cost of *t1_p has lower avg cost than *t2_p
 *             0  if they have equal avg cost
 *             1  if *t1_p has greater avg cost
 * Note:       For use by qsort library function
 */
int Compare_avg_cost(const void* t1_p, const void* t2_p) {
   tour_t t1 = *((tour_t*) t1_p);
   ...
   if (t1->count == 1)
      ...
      t1_avg_cost = ((float) t1->cost)/(t1->count-1);

   if (t2->count == 1)
      t2_avg_cost = 0.0;
   else
      t2_avg_cost = ((float) t2->cost)/(t2->count-1);

   if (t1_avg_cost < t2_avg_cost)
      return -1;
   else if (t1_avg_cost == t2_avg_cost)
      return 0;
   else /* t1_avg_cost > t2_avg_cost */
      return 1;
}  /* Compare_avg_cost */


/*------------------------------------------------------------------
 * Function:   Compare_tour_len
 * Purpose:    Compare the length of two tours and determine which
 *             is smaller.  If the two tours have the same length,
 *             then go through the cities until a difference is
 *             found.  At this point the tour with the "larger" city 
 *             is "smaller".  This is done to try to insure that
 *             the order in which the tours are examined won't
 *             be changed by the split.
 * In args:    t1_p, t2_p:  pointers to pointers to two tours
 * Ret val:    -1 if *t1_p is shorter than *t2_p
 *             0  if they have equal lengths  (Can't happen)
 *             1  if *t1_p is longer
 * Note:       For use by qsort library function
 */
int Compare_tour_len(const void* t1_p, const void* t2_p) {
   ...
   if (t1->count < t2->count) {
      ...
      i = 1;
      while (i < t1->count) 
         if (t1->cities[i] > t2->cities[i])
            return -1;
         else if (t1->cities[i] < t2->cities[i])
            return 1;
         else
            i++;
...
/*------------------------------------------------------------------
 * Function:   Cost_split
 * Purpose:    Order tours on the stack by total cost.  Split the 
 *             stack by assigning alternate records of increasing cost
 *             to the new stack
 * In/out arg: stack
 * Ret val:    new_stack
 */
my_stack_t Cost_split(my_stack_t stack) {
   ...
   /* Sort by partial tour cost */
   qsort(stack->list, stack->list_sz, sizeof(tour_t), Compare_tour_cost);
#  ifdef SPLIT_DEBUG
   Print_stack(stack, 0, "After initial sort");
#  endif

   /* Assign alternate tours to new_stack, compressing stack */
   new_stack = Alternating_split(stack);

   /* Sort new_list and old_list by tour length */
   qsort(new_stack->list, new_stack->list_sz, sizeof(tour_t), 
         Compare_tour_len);
   qsort(stack->list, stack->list_sz, sizeof(tour_t), 
         Compare_tour_len);
...
/*------------------------------------------------------------------
 * Function:   Avg_cost_split
 * Purpose:    Order tours on the stack by average cost per edge.
 *             Split by assigning alternate records of increasing
 *             average cost to the new stack
 * In/out arg: stack
 * Ret val:    new_stack
 */
my_stack_t Avg_cost_split(my_stack_t stack) {
   ...
   /* Sort by partial tour average cost */
   qsort(stack->list, stack->list_sz, sizeof(tour_t), Compare_avg_cost);
#  ifdef SPLIT_DEBUG
   Print_stack(stack, 0, "After initial sort");
#  endif
