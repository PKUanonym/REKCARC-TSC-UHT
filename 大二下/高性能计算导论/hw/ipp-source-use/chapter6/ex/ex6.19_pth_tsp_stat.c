 ...
 *           to reuse deallocated tours.  This version uses a read-write
 *           lock to protect the best tour data structure: it's
 *           locked for reading in Best_tour and Feasible;  it's
 *           locked for writing in Update_best_tour
 *
 * Compile:  gcc -g -Wall -o ex6.19_pth_tsp_stat ex6.19_pth_tsp_stat.c -lpthread
 *           Needs timer.h
 * Usage:    ex6.19_pth_tsp_stat <thread count> <matrix_file>
 *
...
city_t home_town = 0;
tour_t best_tour;
int best_tour_updates = 0;
int obtain_wr_lock = 0;
...
#  endif
   ...
   printf("obtain_wr_lock = %d, best_tour_updates = %d\n", 
         obtain_wr_lock, best_tour_updates);
...
int Best_tour(tour_t tour) {
   ...
   better = cost_so_far + Cost(last_city, home_town) < Tour_cost(best_tour);
...
int Feasible(tour_t tour, city_t city) {
   ...
   if (!Visited(tour, city)) {
      pthread_rwlock_rdlock(&best_tour_rwlock);
      better = Tour_cost(tour) + Cost(last_city,city) < Tour_cost(best_tour);
      pthread_rwlock_unlock(&best_tour_rwlock);
      return better;
   } else
      return FALSE;
