 ...
 *           to reuse deallocated tours.  During the search each process 
 *           stores its own private best tour.  When the processes are
 *           done searching the global best tour is found using a global
 *           reduction.
 *
 * Compile:  mpicc -g -Wall -o ex6.21_mpi_tsp_stat ex6.21_mpi_tsp_stat.c 
 * Usage:    mpiexec -n <proc count> ex6.21_mpi_tsp_stat <matrix_file>
 *
 ...
 *    loc_best_tour and loc_best_tour_cost
 */
void Par_tree_search(void) {
   city_t nbr;
 ...
 *
 * Global var in/out:
 *    Process 0:  loc_best_tour_cost
 *    
 * Global var out:
 *    Process 0:  loc_best_tour
 *
 * Global_vars in:
 *    Processes != 0:  loc_best_tour_cost
 *    Process owning global best tour:  loc_best_tour
 */
void Get_global_best_tour(void) {
   struct {
      int cost;
   ...
   loc_data.cost = loc_best_tour_cost;
...
#  endif
   ...
   if (my_rank == 0) {
      ...
      loc_best_tour_cost = global_data.cost;
...
#     endif
   ...
   Set_init_tours(init_tour_count, counts, displacements, 
         &my_count, &tour_list);
// printf("Proc %d > my initial tour count = %d\n", my_rank, my_count);
 ...
 *              n-city input tour will lead to a local best tour.
 * In arg:
 *    tour:     tour visiting all n cities
 * Ret val:
 *    TRUE if local best tour, FALSE otherwise
 */
int Best_tour(tour_t tour) {
   cost_t cost_so_far = Tour_cost(tour);
   city_t last_city = Last_city(tour);

   if (cost_so_far + Cost(last_city, home_town) < loc_best_tour_cost)
      return TRUE;
   else
      return FALSE;
}  /* Best_tour */
 ...
 * Purpose:     Replace the existing local best tour with the input tour +
 *              hometown
 * In arg:
...
void Update_best_tour(tour_t tour) {
   ...
   loc_best_tour_cost = Tour_cost(loc_best_tour);
}  /* Update_best_tour */
...
int Feasible(tour_t tour, city_t city) {
   ...
   if (!Visited(tour, city) && 
        Tour_cost(tour) + Cost(last_city,city) < loc_best_tour_cost)
      return TRUE;
   else
      return FALSE;
}  /* Feasible */
