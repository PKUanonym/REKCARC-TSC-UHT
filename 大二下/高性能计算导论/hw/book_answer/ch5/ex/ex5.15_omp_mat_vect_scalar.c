/* File:     ex5.15_omp_mat_vect_scalar.c 
 *
 *
 * Purpose:  
 ...
 *     This version uses a temporary scalar to store the
 *     updates to y[i].  This version is based on omp_mat_vect_rand_split.c
 *
 * Compile:  
 *    gcc -g -Wall -fopenmp -o ex5.15_omp_mat_vect_scalar 
 *          ex5.15_omp_mat_vect_scalar.c
 * Usage:
 *    ex5.15_omp_mat_vect_scalar <thread_count> <m> <n>
 *
 * Input:
...
#  pragma omp parallel num_threads(thread_count)  \
      default(none) private(i, j, my_rank, sum)  shared(A, x, y, m, n)
   {
