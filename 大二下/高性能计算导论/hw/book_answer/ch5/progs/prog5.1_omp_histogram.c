/* File:      prog5.1_omp_histogram.c
 * Purpose:   Build a histogram from random data using OpenMP
 * 
 * Compile:   gcc -g -Wall -fopenmp -o prog5.1_omp_histogram 
 *                  prog5.1_omp_histogram.c
 * Run:       ./histogram <bin_count> <min_meas> <max_meas> 
...
#           ifdef DEBUG
            printf("Th %d > Adding %d to bin_counts[%d] = %d\n",
                  my_rank, loc_bin_counts[j*bin_count + i],
                  i, bin_counts[i]);
#           endif
