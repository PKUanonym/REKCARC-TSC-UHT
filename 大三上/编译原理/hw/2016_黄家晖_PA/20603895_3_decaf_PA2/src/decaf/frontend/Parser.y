...
Expr            :	LValue
                ...
                |   Expr '?' Expr ':' Expr
                    {
                    	$$.expr = new Tree.Ternary(Tree.CONDEXPR, $1.expr, $3.expr, $5.expr, $2.loc);
                   	}
                |   Expr PCLONE Expr
                    {
...
SwitchStmt      :   SWITCH '(' Expr ')' '{' CaseStmtList DefaultStmt '}'
                    {
                        $$.stmt = new Tree.Switch($3.expr, $6.caselist, $7.slist, $1.loc);
                    }
                ;
...
CaseStmt        :   CASE Constant ':' StmtList
                    {
                        $$.casedef = new Tree.Case($2.expr, $4.slist, $1.loc);
                    }
                ;
...
RepeatStmt      :   REPEAT StmtList UNTIL '(' Expr ')'
                    {
                        $$.stmt = new Tree.Repeat($5.expr, new Tree.Block($2.slist, $1.loc), $1.loc);
                    }
                ;
