...
public class Translater {
	...
	public void printTo(PrintWriter pw) {
		for (VTable vt : vtables) {
			...
			pw.println("    " + vt.actualClass.getCopyFuncLabel().name + ";");
	...
	public void genCheckDivByZero(Temp divisor) {
		Label exit = Label.createLabel();
		Temp cond = genEqu(divisor, genLoadImm4(0));
		genBeqz(cond, exit);
	...
	public void genCopyForClass(Class c) {
		currentFuncty = new Functy();
		currentFuncty.label = Label.createLabel(
				"_" + c.getName() + "." + "COPY", true);
		Temp template = Temp.createTempI4();
		c.setCopyFuncLabel(currentFuncty.label);
		currentFuncty.paramMemo = Tac.genMemo(template.name + ":4");
		genMark(currentFuncty.label);
		Temp size = genLoadImm4(c.getSize());
		genParm(size);
		Temp newObj = genIntrinsicCall(Intrinsic.ALLOCATE);
		...
		if (time != 0) {
			for (int i = 0; i < time; i++) {
				int offset = OffsetCounter.WORD_SIZE * (i + 1);
				genStore(genLoad(template, offset), newObj, offset);
			}
		}
		genStore(genLoadVTable(c.getVtable()), newObj, 0);
		genReturn(newObj);
