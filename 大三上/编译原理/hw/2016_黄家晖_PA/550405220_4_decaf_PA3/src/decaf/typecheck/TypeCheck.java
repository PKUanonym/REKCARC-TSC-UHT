...
public class TypeCheck extends Tree.Visitor {
	...
	public void visitTernary(Tree.Ternary expr) {
		// Check Sub-operands' types
		expr.left.accept(this);
		expr.right.accept(this);
		expr.middle.accept(this);

		// Assert: Only Ternary Type
		if (expr.tag != Tree.CONDEXPR) {
			expr.type = BaseType.ERROR;
			return;
		}

		checkTestExpr(expr.left);

		Type returnType = BaseType.ERROR;
		if (expr.right.type.compatible(expr.middle.type)) {
			returnType = (expr.middle.type == BaseType.ERROR ||
					expr.right.type == BaseType.ERROR) ?
					BaseType.ERROR : expr.middle.type;
		} else {
			issueError(new IncompatCondExprError(expr.left.loc,
					expr.middle.type.toString(),
	...
	public void visitRepeat(Tree.Repeat repeatLoop) {
		...
		// actions is not null (default an empty block)
	...
	public void visitSwitch(Tree.Switch switchStmt) {
		...
		if (!BaseType.INT.compatible(switchStmt.condition.type)) {
			issueError(new SwitchVarNotInt(switchStmt.condition.loc));
		}
	...
	public void visitCase(Tree.Case caseStmt) {
		// case (ERROR) stmt will trigger error inside.
		caseStmt.condition.accept(this);
		if (!(caseStmt.condition instanceof Tree.Literal) ||
				!(BaseType.INT.compatible(caseStmt.condition.type))) {
			issueError(new CaseLabelNotConstError(caseStmt.condition.loc));
		}
		// No need to open table.
	...
	private Type checkBinaryOp(Tree.Expr left, Tree.Expr right, int op, Location location) {
		...
		case Tree.PCLONE:
			if (left.type.isClassType() && right.type.isClassType()) {
				ClassType MCA = getMinCommonAncestor((ClassType) left.type,
						(ClassType) right.type);
				if (MCA != null) {
					compatible = true;
					returnType = MCA;
				}
	...
	private ClassType getMinCommonAncestor(ClassType a, ClassType b) {
		for (ClassType ap = a; ap != null; ap = ap.getParentType()) {
			for (ClassType bp = b; bp != null; bp = bp.getParentType()) {
				if (bp.equal(ap)) return ap;
			}
		}
		return null;
	}
