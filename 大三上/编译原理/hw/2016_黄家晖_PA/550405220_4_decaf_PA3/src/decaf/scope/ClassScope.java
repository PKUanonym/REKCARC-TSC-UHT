...
public class ClassScope extends Scope {
	...
	public void printTo(IndentPrintWriter pw) {
		TreeSet<Symbol> ss = new TreeSet<>(Symbol.LOCATION_COMPARATOR);
		ss.addAll(symbols.values());
		pw.println("CLASS SCOPE OF '" + owner.getName() + "':");
		pw.incIndent();
		ss.forEach(pw::println);
		ss.stream().filter(Symbol::isFunction).forEach(symbol ->
