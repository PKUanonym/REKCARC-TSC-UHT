...
/*
static void
print_page_links(struct Page* p1, struct Page* p2, struct Page* p3) {
    cprintf("+ free_list = %x, free_list.next = %x, prev = %x\n", &(free_list), free_list.next, free_list.prev);
    cprintf("+ 0 = %x, 0.next = %x, prev = %x\n", &(p1->page_link), p1->page_link.next, p1->page_link.prev);
