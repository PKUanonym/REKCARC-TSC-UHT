...
def parse_word(filename, index):
	global xmlfile
	if (os.path.isfile(filename)):
		# child = subprocess.Popen(['pdf2txt.py', filename], stdout=subprocess.PIPE)
		...
		try:
			...
			return
		xmlfile.write( '\t\t<doc title=\"'+'DOCX'+'\" url=\"'+filename[7:].decode('gbk')+'\" id=\"'+index+'\" pr=\"0.0\">\n')
		p = process_content(text)
...
if __name__ == '__main__':
	...
	for index in file_dict.keys():
		...
		if temp[-1] == 'docx':
			print(filename)
			parse_word('mirror/'+filename, index)
			# print filename
	xmlfile.write('\t</urls>\n')
	xmlfile.write('</xml>\n')
